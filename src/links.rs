...
impl<'a> VerifyOptions<'a> {
    pub fn new(github_token: Option<&'a str>, github_api_base_url: Url) -> Self {
        debug_assert!(github_api_base_url.host_str().is_some());
        debug_assert!(github_api_base_url.query().is_none());
        debug_assert!(github_api_base_url.fragment().is_none());
        Self {
            github_token,
            github_api_base_url,
        }
...
pub async fn verify_with_options(
    ...
    for url in &all_urls {
        ...
        if let (Some(token), Some(parsed_url)) = (github_token, parsed.as_ref())
            && let Some(api_path) = github_api_path_for(parsed_url, &options.github_api_base_url)
        {
        ...
        if let Some(reason) =
            verify_generic(&generic_client, url, is_github_url, github_token).await
        {
            warn!("broken link: {url} ({reason})");
            broken.push((url.clone(), reason));
        }
...
async fn verify_generic(
    ...
    match client.head(url).send().await {
        Ok(resp) if should_report_status(resp.status(), is_github_url) => {
            Some(status_reason(resp.status(), is_github_url, github_token))
        }
...
fn status_reason(status: StatusCode, is_github_url: bool, github_token: Option<&str>) -> String {
    if is_github_url {
        github_browser_status_reason(status, github_token.is_some())
    } else {
